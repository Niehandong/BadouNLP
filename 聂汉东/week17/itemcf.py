...
def item_cf(user_id, item_id, similar_items, user_to_rating, topn=10):
    ...
    count = 0
    # 获取目标用户对所有物品的评分
    user_ratings = user_to_rating[user_id]

    # 遍历与目标物品最相似的前topn个物品
    for similar_item, similarity in similar_items[item_id][:topn]:
        # 用户对相似物品的评分
        rating_for_similar_item = user_ratings[similar_item - 1]

        # 如果用户对相似物品有评分
        if rating_for_similar_item != 0:
            # 分数*物品相似度，作为加权
            pred_score += rating_for_similar_item * similarity
            count += 1
